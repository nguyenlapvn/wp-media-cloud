<?php
namespace Advanced_Media_Offloader\Vendor\Aws\Credentials;

use Aws;
use Advanced_Media_Offloader\Vendor\Aws\Api\DateTimeResult;
use Advanced_Media_Offloader\Vendor\Aws\CacheInterface;
use Advanced_Media_Offloader\Vendor\Aws\Exception\CredentialsException;
use Advanced_Media_Offloader\Vendor\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \Advanced_Media_Offloader\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \Advanced_Media_Offloader\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \Advanced_Media_Offloader\Vendor\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($credFile)) {
            $profiles = \Advanced_Media_Offloader\Vendor\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \Advanced_Media_Offloader\Vendor\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
