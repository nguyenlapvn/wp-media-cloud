...
use Advanced_Media_Offloader\Vendor\Aws\Arn\ArnParser;
use Advanced_Media_Offloader\Vendor\Aws\Arn\ObjectLambdaAccessPointArn;
...
use Advanced_Media_Offloader\Vendor\Aws\Arn\S3\OutpostsAccessPointArn;
use Advanced_Media_Offloader\Vendor\Aws\Arn\S3\MultiRegionAccessPointArn;
use Advanced_Media_Offloader\Vendor\Aws\Arn\S3\OutpostsArnInterface;
...
use Advanced_Media_Offloader\Vendor\Aws\Exception\UnresolvedEndpointException;
use Advanced_Media_Offloader\Vendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \Advanced_Media_Offloader\Vendor\Aws\strip_fips_pseudo_regions($region);
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(
                    \Advanced_Media_Offloader\Vendor\Aws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
