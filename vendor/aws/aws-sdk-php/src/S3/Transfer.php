...
class Transfer implements PromisorInterface
{
    ...
    private function createUploadPromise()
    {
        ...
        $files = \Advanced_Media_Offloader\Vendor\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \Advanced_Media_Offloader\Vendor\Aws\filter(
                \Advanced_Media_Offloader\Vendor\Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = \Advanced_Media_Offloader\Vendor\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return \Advanced_Media_Offloader\Vendor\Aws\filter($files, function ($key) {
